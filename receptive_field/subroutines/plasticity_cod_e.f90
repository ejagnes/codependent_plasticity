...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	IMPLICIT NONE
	INTEGER		::	j0,jf
!======== block of excitatory plasticity if inhibition is too high ========================!
	IF(x(19).LT.p_plast(36)) THEN
		x(24) = x(19)
	ELSE
		x(24) = 100000000.0d0
	END IF
...
!============================= finding which input neurons spiked =========================!
	WHERE(spk(j0:jf))
		...
		msynw(j0:jf) = msynw(j0:jf)*(1.0d0 - p_plast(16)*ypost(j0:jf)*DEXP(-(x(24)/p_plast(18))**p_plast(19)))	!update weights
	END WHERE
!==========================================================================================!
!=========================== updates when post neuron spikes ==============================!
	IF(spk_post) THEN
		...
		msynw(j0:jf) = msynw(j0:jf) + (p_plast(15)*ypre(j0:jf)*x(18) - p_plast(17)*x(2)*(x(18)**2))*DEXP(-(x(24)/p_plast(18))**p_plast(19))	!update weights
